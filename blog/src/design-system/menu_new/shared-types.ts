 ...
 */
export type MenuItem = MenuItemSelf
 ...
 * menu.
 */
export function isClosableKey(code: string): code is Values<typeof KEYDOWN_CLOSE_KEYS> {
 ...
 * menu item.
 */
export function isSelectableKey(code: string): code is Values<typeof SELECTION_KEY> {
...
export function isElementInSubtree(
  ...
  const focusEv = new Event('element-contains', { bubbles: true, composed: true });
  let composedPath: EventTarget[] = [];
  const listener = (ev: Event) => {
    composedPath = ev.composedPath();
  };

  container.addEventListener('element-contains', listener);
  target.dispatchEvent(focusEv);
  container.removeEventListener('element-contains', listener);
